...
use strict;
use lib "/usr/sbin/keysurvey/ctes/lib";
use Net::SMTP;
...
########## Variables #########
my $VERSION = "1.2";			# Version of module
...
sub SendNotification(@)
{
	...
	$sender->datasend("Subject: Statistic from KSCTES utility.\n");
	$sender->datasend("\n");
...
# Checks for errors which were occure during the work
...
# Returned messages. Populates the array by issues
sub ReturnedMessage(@)
{
	my $self = shift;
	my $message = shift @_;
...
# Description of errors. Get the issue description (understandable for human).
